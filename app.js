...
function generatePdf(doc, tipo) {
  ...
  pdf.setFontSize(14);
  pdf.text(config.nombreNegocio || "Nexus Finance", 50, 20);
  pdf.setFontSize(10);
  if (config.direccion) pdf.text(config.direccion, 50, 26);
  if (config.telefono) pdf.text(`Tel: ${config.telefono}`, 50, 32);
  if (config.email) pdf.text(config.email, 50, 38);

  y += 5;
  pdf.setFontSize(16);
  pdf.text(tipo.toUpperCase(), 10, y);
  y += 8;

  pdf.setFontSize(10);
  pdf.text(`Número: ${doc.numero}`, 10, y);
  pdf.text(`Fecha: ${doc.fecha}`, 100, y);
  y += 6;
  pdf.text(`Cliente: ${doc.cliente}`, 10, y);
  y += 6;
  if (doc.direccion) {
    pdf.text(`Dirección: ${doc.direccion}`, 10, y);
    y += 6;
  }
  if (doc.email) {
    pdf.text(`Email: ${doc.email}`, 10, y);
    y += 6;
  }
  if (doc.telefono) {
    pdf.text(`Teléfono: ${doc.telefono}`, 10, y);
    y += 8;
  }
  ...
  doc.items.forEach((it) => {
    ...
    pdf.text(String(it.descripcion || ""), 10, y);
    pdf.text(String(it.cantidad || 0), 92, y);
    pdf.text(formatMoney(it.precio || 0), 110, y);
    pdf.text(`${it.impuesto || 0}%`, 140, y);
    pdf.text(formatMoney(linea), 170, y);
    y += 6;
  });
