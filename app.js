...
function setupConfig() {
  ...
  logoInput?.addEventListener("change", (e) => {
    ...
    reader.onload = () => {
      ...
      // Determinar ratio real del logo para no estirarlo
      const img = new Image();
      img.onload = () => {
        if (img.width && img.height) {
          config.invoiceLogoRatio = img.height / img.width; // alto/ancho
        } else {
          config.invoiceLogoRatio = 0.4; // fallback
        }
        config.invoiceLogoBase64 = invoiceLogoBase64;
        saveConfig();
...
function generateFacturaPdf(factura) {
  ...
  if (invoiceLogoBase64) {
    try {
      const logoWidth = 35; // ancho fijo en mm
      const ratio = config.invoiceLogoRatio && config.invoiceLogoRatio > 0
        ? config.invoiceLogoRatio
        : 0.4; // fallback
      const logoHeight = logoWidth * ratio;
      doc.addImage(invoiceLogoBase64, "PNG", marginLeft, 10, logoWidth, logoHeight);
      cursorY = 10 + logoHeight + 5;
    } catch (e) {
